...
const KB = {
  ...
  payment: `We accept: bKash · Nagad · Credit/Debit Card · Bank Transfer · Cash on Delivery.
    All payments are processed securely through our encrypted checkout.`,
  ...
  faq: [
    ...
    { triggers: ['payment','pay','bkash','nagad','card','cash'],
      response: `We accept bKash, Nagad, Credit/Debit Card, Bank Transfer, and Cash on Delivery. All payments go through our encrypted checkout system.` },
...
function renderProducts(list) {
  ...
  grid.innerHTML = list.map((p, i) => {
    ...
    return `
    <article class="product-card reveal-up ${i > 0 ? `delay-${i}` : ''}">
      <div class="product-card-img">
        ...
        <button class="quick-view-overlay" onclick="quickView('${p.perfume_name}', '${p.brand_name || ''}')">
          Quick View
        </button>
      </div>
      <div class="product-card-body">
        ...
        <button class="add-btn" onclick="addToCart(this, '${p.perfume_name}')">Add to Cart</button>
      </div>
    </article>`;
  }).join('');
...
resultsEl.innerHTML = results.map((p, idx) => `
    <div class="search-result-row" role="option" tabindex="0" data-index="${idx}" data-perfume-name="${p.perfume_name}" data-brand-name="${p.brand_name || p.brand || ''}">
      <div class="sr-info">
