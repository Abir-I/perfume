...
const SHOP_FALLBACK_IMG = 'https://images.unsplash.com/photo-1541643600914-78b084683702?w=500&q=80&auto=format';
...
function renderShopGrid(products) {
  ...
  shopGrid.innerHTML = products.map(p => `
    <article class="shop-card" data-product-id="${p.product_id}" onclick="quickView('${(p.perfume_name || '').replace(/'/g, "\\'")}', '${(p.brand_name || '').replace(/'/g, "\\'")}')">
      <div class="shop-card-img">
        <img
          src="${p.image_url || SHOP_FALLBACK_IMG}"
          alt="${p.perfume_name}"
          loading="lazy"
          onerror="this.src='${SHOP_FALLBACK_IMG}'"
        />
      </div>
