...
Resolves JWT tokens against the accounts.User table instead of Django's
built-in auth.User table. This is required because LoginView issues tokens
against accounts.User, but DRF's default JWTAuthentication resolves them
against the built-in table — a mismatch.
"""
