...
class AdminPerfumeCreateView(APIView):
    ...
    def post(self, request):
        ...
        if concentration not in VALID_CONCENTRATIONS:
            errors['concentration'] = f'Must be one of {sorted(VALID_CONCENTRATIONS)}.'
        ...
        recommended_season = data.get('recommended_season') or None
        if recommended_season and recommended_season not in VALID_SEASONS:
            return Response({'errors': {'recommended_season': f'Must be one of {sorted(VALID_SEASONS)}.'}},
...
class AdminPerfumeUpdateView(APIView):
    ...
    def patch(self, request, perfume_id):
        ...
        if data.get('concentration'):
            if data['concentration'] not in VALID_CONCENTRATIONS:
                return Response({'errors': {'concentration': f'Must be one of {sorted(VALID_CONCENTRATIONS)}.'}},
                                 status=status.HTTP_400_BAD_REQUEST)
        ...
        if data.get('recommended_season'):
            if data['recommended_season'] not in VALID_SEASONS:
                return Response({'errors': {'recommended_season': f'Must be one of {sorted(VALID_SEASONS)}.'}},
                                 status=status.HTTP_400_BAD_REQUEST)
